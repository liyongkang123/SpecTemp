...
# --- Use @overload to define different function signatures ---

# Signature 1: When return_scores is True
@overload
def mrr(
...
# Signature 2: When return_scores is False (or not provided)
@overload
def mrr(
    qrels: dict[str, dict[str, int]],
    results: dict[str, dict[str, float]],
...
# The implemented type annotation uses Union, because it must be able to handle all possible cases
def mrr(
    qrels: dict[str, dict[str, int]],
    results: dict[str, dict[str, float]],
    k_values: list[int],
    ...
    ## Get the top-k_max documents for each query
        ...
        for k in k_values:
            ...
            for rank, hit in enumerate(top_hits[query_id][0:k]):
                if hit[0] in query_relevant_docs:
                    rr = 1.0 / (rank + 1)
                    break
            MRR[f"MRR@{k}"] += rr # Save the sum of the reciprocal rank scores of all queries
